...
async def start(bot, message):
  ...
  keyboard = [
    [
      InlineKeyboardButton("馃殌 Physics Wallah without Purchase 馃殌", callback_data="pwwp")
    ],
    [
      InlineKeyboardButton("馃摌 Classplus without Purchase 馃摌", callback_data="cpwp")
    ],
    [
      InlineKeyboardButton("馃搾 Appx Without Purchase 馃搾", callback_data="appxwp")
    ]
  ]
                                ...
                                async with session.get(f"https://api.classplusapp.com/v2/course/preview/org/info", params=params, headers=batch_headers) as response:
                                    if response.status == 200:
                                        ...
                                        if course_content:
                                            ...
                                            caption = f"**App Name : ```\n{App_Name}({org_code})```\nBatch Name : ```\n{selected_batch_name}``````\n馃幀 : {video_count} | 馃搧 : {pdf_count} | 馃柤  : {image_count}``````\nTime Taken : {formatted_time}```**"
